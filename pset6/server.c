...
char* indexes(const char* path)
{
  ...
  // length of path plus null byte
    int pathNum = strlen(path) + 1;
    int indexLength = strlen("index.html");
    
    char indexPath[pathNum];
    strcpy(indexPath, path);


    // allocate memory on heap 
    char* pathMem = malloc(pathNum + indexLength);
    if (pathMem == NULL)
    {
        return NULL;
    }
    //if index.html or index.php exist append to path
    if ((strstr(indexPath, "index.html") != NULL) || (strstr(indexPath, "index.php") != NULL))
    {
        strcpy(pathMem, indexPath);
        return pathMem;
        
    }
    else if (strcmp(&indexPath[pathNum - 2], "/") != 0)
    {
        strcpy(pathMem, indexPath);
        strcat(pathMem, "/");
    }
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    //read file by charactor storing in buffer
    //resizing buffer as charactors are added
    while(!feof(file))
    {
        char info  = fgetc(file);
        buffer[counter] =  info ;    
        counter++;
        buffer = realloc(buffer, sizeof(BYTE) * (counter + 1));
...
const char* lookup(const char* path)
{
    ...
    //printf("%s", path);    
    if(strstr(haystack, ".css"))
    {
        return "text/css";
    }
    else if(strstr(haystack, ".html"))
    {
        return "text/html";
    }
    else if(strstr(haystack, ".gif"))
    {
        return "image/gif";
    }
    else if(strstr(haystack,".ico"))
    {
        return "image/x-icon";
    }
    else if(strstr(haystack, ".js"))
    {
        return "text/javascript";
    }
    else if(strstr(haystack, ".php"))
    {
        return "text/x-php";
    }
    else if(strstr(haystack, ".png"))
    {
        return "image/png";
    }
    else if(strstr(haystack, ".jpg"))
    {
        return "image/jpg";
    }
    else
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char* space = strstr(haystack, " ");
    //int scheme = strcmp(space, "HTTP/1.1"); 
    char* slash  = strstr(haystack, "/");
    char* secondSpace = strstr(slash, " ");
    int placeHolder = 0;
    //const char* scheme1 = strtok(space, scheme);
   printf("%s\n",secondSpace);
    //check for valid request line
    if(strstr(haystack, "GET") == NULL || haystack[0] != 'G' || haystack[3] != ' ')
    {
        error(405);
        return false;
    }
    if(strstr(haystack, "/") == NULL || space[1] != '/')
    {
        error(501);
        return false;
    }
    if(strstr(haystack, "HTTP/1.1\r\n") == NULL || secondSpace[1] != 'H') 
    {
        error(505);
        return false;
    }
    ...
    space = space + 1;
    placeHolder = strstr(space, " ") - space;
    char position[placeHolder];
    strncpy(position, space, placeHolder);
    ...
    {
        space = strstr(position, "?");
        placeHolder = space - position;
        strncpy(abs_path, position, placeHolder);
        strcpy(query, space + 1);
    }
    //check for query value in request line
    else if (strstr(position, "q=") != NULL) 
    {
        space = strstr(position, "q=");
        placeHolder = space - position;
        strncpy(abs_path, position, placeHolder);
        strcpy(query, space);
    }    
